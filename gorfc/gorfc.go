...
func fillVariable(cType C.RFCTYPE, container C.RFC_FUNCTION_HANDLE, cName *C.SAP_UC, value interface{}, typeDesc C.RFC_TYPE_DESC_HANDLE) (err error) {
	...
	case C.RFCTYPE_TIME:
		...
		case string:
			...
			cLen := C.uint(C.GoStrlenU((*C.SAP_UTF16)(cValue)))
			rc = C.RfcSetChars(container, cName, (*C.RFC_CHAR)(cValue), cLen, &errorInfo)
			// cValue, err = fillString(v)
			// rc = C.RfcSetTime(container, cName, (*C.RFC_CHAR)(cValue), &errorInfo)
		case time.Time:
			cValue, err = fillString(v.Format("150405"))
			rc = C.RfcSetTime(container, cName, (*C.RFC_CHAR)(cValue), &errorInfo)
		default:
			var goName string
			goName, err = wrapString(cName, true)
...
// IsValidDateYYMMDD checks if a string is a valid date in the YYMMDD format.
func IsValidDateYYMMDD(dateStr string) bool {
	layout := "20060102" // YYYYMMDD format (Go uses 2006 for year, 01 for month, and 02 for day)
	_, err := time.Parse(layout, dateStr)
	return err == nil
}

// IsValidTimeHHMMSS checks if a string is a valid time in the HHMMSS format.
func IsValidTimeHHMMSS(timeStr string) bool {
	layout := "150405" // HHMMSS format (Go uses 15 for hour, 04 for minute, and 05 for second)
